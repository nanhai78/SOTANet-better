...
if __name__ == "__main__":
    ...
    input_shape = [640, 640]
    # ------------------------------------------------------#
    #   所使用的版本有"Medium"和"Large", Large版本的channel数量翻倍
    # ------------------------------------------------------#
    model_size = "Medium"
    # ------------------------------------------------------------------#
    #   mosaic              马赛克数据增强。
    ...
    #   获得图片路径和标签
    # ----------------------------------------------------#
    train_annotation_path = 'trainSet.txt'
    val_annotation_path = 'valSet.txt'

    # ------------------------------------------------------#
    #   设置用到的显卡
    # ------------------------------------------------------#
    ngpus_per_node = torch.cuda.device_count()  # 计算当前设备上可用的显卡
    if distributed:
    ...
    #   创建yolo模型
    # ------------------------------------------------------#
    model = YoloBody(num_classes, model_size, depthwise=False)
    weights_init(model)
    if model_path != '':
        # ------------------------------------------------------#
        #   权值文件请看README，百度网盘下载
    ...
    #   获得损失函数
    # ----------------------#
    yolo_loss = YOLOLoss(num_classes, fp16, focal_loss=focal_loss)
    # ----------------------#
    #   记录Loss
    # ----------------------#
    if local_rank == 0:
        time_str = datetime.datetime.strftime(datetime.datetime.now(), '%Y_%m_%d_%H_%M_%S')  # 获取当前时间
        log_dir = os.path.join(save_dir, "loss_" + str(time_str))  # 日志文件夹名称
    ...
    # ------------------------------------------------------------------#
    if fp16:  # 导入GradScaler,用于后面梯度的放缩
    ...
    if Cuda:
        if distributed:
            ...
            cudnn.benchmark = True  # inference 优化卷积算法
    ...
    if local_rank == 0:
        ...
        total_step = num_train // Unfreeze_batch_size * UnFreeze_Epoch  # 总共的step
