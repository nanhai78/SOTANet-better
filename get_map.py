...
if __name__ == "__main__":
    ...
    # 获取test.txt文件里面的文件名
    image_ids = open(os.path.join(Mydataset_path, "Main/test.txt")).read().strip().split()  # 指向test.txt文件

    if not os.path.exists(map_out_path):  # 创建map_out
    ...
    # 将每副图像预测的结果写入detection_results下的txt文件
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):  # tqdm进度条库如image_ids长度为1800,在控制台就会显示处理图像的进度
            ...
            yolo.get_map_txt(image_id, image, class_names, map_out_path)  # 将预测的结果写入detection_results下的txt文件
            ...
            with open(os.path.join(map_out_path, "ground-truth/" + image_id + ".txt"),
                ...
                for obj in root.findall('object'):
                    ...
                    if difficult_flag:
                        ...
                        new_f.write("%s %s %s %s %s\n" % (obj_name, left, top, right, bottom))  # 写入类别名称 和坐标信息
        print("Get ground truth result done.")

    if map_mode == 0 or map_mode == 3:
        print("Get map.")
        get_map(MINOVERLAP, True, score_threhold=score_threhold, path=map_out_path, nms=nms_iou)
        print("Get map done.")
